...
export function googleGenerateContentAPIModel<zO, zI>(options: GoogleGenerateContentAPIModelOptions): Adapter<zO, zI> {
  ...
  async function getHistory(history: ChatItem[], toolMap: GoogleToolMap[], signal: AbortSignal): Promise<Content[]> {
    ...
    for (const item of history) {
      switch (item.type) {
        ...
        case "tool_result_file": {
          ...
          const fileName = await ensureFileUploaded(item.content, item.kind, signal);
          googleHistory.push({
            role: "user",
            parts: [{
              fileData: {
                fileUri: new URL(fileName, getGoogleFileBaseUrl(baseUrl)).toString(),
                mimeType: item.kind,
              },
            }],
          });
  ...
  return {
    ...
    stream: async function* stream<zO, zI>({
      ...
      const googleHistory = await getHistory(history, normalizedTools, signal);
