...
function traceSnapshot(events: TraceEvent[]) {
  ...
  return events.map((event) => ({
    ...
    rootParent: event.rootParent == null ? null : indexById.get(event.rootParent),
...
Deno.test("global tracer captures tool turns, message spans, and token counts", async () => {
  ...
  assertEquals(agentTrace.content, { name: undefined });
...
Deno.test("local tracer captures sub-agent spans and tags history items with the correct traces", async () => {
  ...
  const toolTrace = filterTrace(events, "tool")[0];
  const innerAgentTrace = filterTrace(events, "agent").find((trace) => trace.content.name === "inner-model");
  const outerOutputMessage = filterTrace(events, "message").find((trace) =>
    trace.parent ===
      filterTrace(events, "model").find((event) => event.content.reason === "tool")?.id
  );
...
Deno.test("tool_use_start hints create message spans without changing tool trace history tags", async () => {
  ...
  const initialModel = filterTrace(events, "model").find((trace) => trace.content.reason === "init");
  const toolTrace = filterTrace(events, "tool")[0];
  const toolUseMessage = filterTrace(events, "message").find((trace) => trace.content.type === "tool_use");
...
Deno.test("provider retries after partial output keep failed message spans", async () => {
  ...
  assertEquals(run.outputText, "partialworked");
  assertStartsMatchEvents(starts, events);
  ...
  const model = new TraceTestModel({
    ...
    stream: () => {
      ...
      return convertChatItemsToStream({
        items: [{
          type: "output_text",
          content: calls === 1 ? JSON.stringify({ name: 123 }) : JSON.stringify({ name: "bingus" }),
        }],
        inputTokens: 1,
        outputTokens: 1,
      });
