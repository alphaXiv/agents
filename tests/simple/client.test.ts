...
Deno.test("addStreamItem keeps chat history dense while reordering", () => {
  const output: ChatItem[] = [];

  addStreamItem(output, {
  ...
  assertEquals(output, [
    { type: "output_reasoning", content: "Hello" },
    { type: "output_text", content: "later" },
  ]);
  ...
  assertEquals(output, [
    { type: "output_reasoning", content: "Hello" },
    { type: "output_text", content: "middle" },
